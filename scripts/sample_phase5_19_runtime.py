...
def get_status() -> dict[str, Any]:
    with urllib.request.urlopen("http://127.0.0.1:8000/status?camera_id=camera_01", timeout=3) as response:
...
def get_process_cpu_seconds(pid: int | None) -> float | None:
    if not pid:
        return None
    try:
        output = ps_text(
...
def summarize(samples: list[dict[str, Any]], failures: list[dict[str, Any]], started_at: float) -> dict[str, Any]:
    ...
    return {
        ...
        "main_frame_age_over_3000_count": count_truthy([(sample.get("main_frame_age_ms") or 0) > 3000 for sample in valid]),
        "analysis_max_frame_age_ms": maximum([sample.get("analysis_frame_age_ms") for sample in valid]),
        "analysis_frame_age_over_3000_count": count_truthy([(sample.get("analysis_frame_age_ms") or 0) > 3000 for sample in valid]),
...
def run(duration_sec: int, output_path: Path) -> None:
    ...
    for index in range(duration_sec):
        ...
        if index % 5 == 0:
            ...
            sample["vision_cpu_seconds"] = get_process_cpu_seconds(vision_pid)
