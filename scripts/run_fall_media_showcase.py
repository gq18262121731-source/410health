...
def process_frame(
    ...
    processed: int,
    max_score: float,
    counters: tuple[dict[str, int], dict[str, int], dict[str, int], dict[str, int]],
    ...
    for bucket, value in zip(counters, states):
    ...
    writer.write(frame)
    return last_result, processed, max_score
...
def run_video(service: FallFrameTestService, source: Path, output: Path, args: argparse.Namespace) -> dict[str, Any]:
    ...
    counters = (raw_counts, status_counts, event_counts, alarm_counts)
    ...
    with tqdm(total=progress_total, unit="frame", desc="fall-detect") as bar:
        while args.max_frames <= 0 or frame_index < args.max_frames:
            ...
            if not ok:
                break
            last_result, processed, max_score = process_frame(
                service, frame, writer, smoother, args, frame_index, fps,
                last_result, processed, max_score, counters, frame_records, review_frames, review_dir,
            )
            frame_index += 1
            bar.update(1)
