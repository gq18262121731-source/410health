...
class DeterministicTemporalTransformer:
    """A deterministic single-head temporal attention scorer for local health sequences."""
    ...
    def describe(self) -> dict[str, object]:
        return {
            "model_name": "deterministic_temporal_transformer",
...
class IntelligentAnomalyScorer:
    ...
    def __init__(
        ...
        *,
        ...
        self._transformer = DeterministicTemporalTransformer(window_size=sequence_length)

    def describe_model(self) -> dict[str, object]:
        description = self._transformer.describe()
    ...
    def build_alarm(self, sample: HealthSample, result: IntelligentAnomalyResult) -> AlarmRecord | None:
        ...
        return AlarmRecord(
            ...
            metadata={
                ...
                "feature_names": list(self._transformer.feature_names),
    ...
    def _score_windows(self, windows: list[list[float]], device_mac: str | None) -> IntelligentAnomalyResult:
        ...
        current = windows[-1]
        transformer = self._transformer.assess(
            windows,
            baseline=baseline,
        ...
        fused_score = round((drift_score * 0.45) + (transformer.temporal_score * 0.55), 4)
        score_probability = max(0.01, min(0.99, fused_score / 4.0))
        probability = max(
            0.01,
            min(
                0.99,
                round((transformer.anomaly_probability * 0.65) + (score_probability * 0.35), 4),
            ),
        )
        reason = self._build_reason(
            windows=windows,
            dominant_features=transformer.dominant_features,
            feature_contributions=transformer.feature_contributions,
        )
        return IntelligentAnomalyResult(
            probability=probability,
            score=fused_score,
            drift_score=drift_score,
            reconstruction_score=transformer.temporal_score,
            reason=reason,
            attention_weights=transformer.attention_weights,
            feature_contributions=transformer.feature_contributions,
