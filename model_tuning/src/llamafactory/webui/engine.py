...
class Engine:
    ...
    def _update_component_with_locale(
        self, lang: str, input_dict: dict[str, dict[str, Any]]
    ) -> dict["Component", "Component"]:
        r"""Update labels and initial values in one pass to avoid generic Gradio labels on first paint."""
        output_dict: dict[Component, Component] = {}
        updated_elem_ids: set[str] = set()

        for elem_name, elem in self.manager.get_elem_iter():
            elem_id = self.manager.get_id_by_elem(elem)
            elem_attr: dict[str, Any] = {}
            if elem_name in LOCALES:
                elem_attr.update(LOCALES[elem_name][lang])

            if elem_id in input_dict:
                elem_attr.update(input_dict[elem_id])
                updated_elem_ids.add(elem_id)

            if elem_attr:
                output_dict[elem] = elem.__class__(**elem_attr)

        for elem_id, elem_attr in input_dict.items():
            if elem_id in updated_elem_ids:
                continue
    ...
    def resume(self):
        ...
        yield self._update_component_with_locale(lang, init_dict)
