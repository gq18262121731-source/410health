...
import { ApiError, api, type AuthAccountPreview, type SessionUser } from "../api/client";
...
export function useSessionAuth() {
  ...
  function fillMockAccount() {
    const account = authAccounts.value.find((item) => item.username === loginHelperAccount.value);
    if (!account) return;
  ...
  async function loadMockAccounts() {
    ...
    loginHelperAccount.value = authAccounts.value[0]?.username ?? "";
  }
