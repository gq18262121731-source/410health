...
<template>
  <section class="page-stack">
    ...
    <section class="member-device-grid">
      <article class="panel member-device-panel member-topology-panel">
        <div class="panel-head">
          <div>
            <p class="section-eyebrow">社区监护总览</p>
            <h2>老人监护对象</h2>
            <p class="subtle-copy">
              先按老人查看绑定状态；只有已绑定设备的老人，点进去后才会加载实时曲线和监护数据。
            </p>
          </div>
          <div class="badge-row">
            <span class="summary-badge">关系链 {{ workspace.relationTopology.value?.lanes.length ?? 0 }}</span>
            <span class="summary-badge">未归属设备 {{ workspace.relationTopology.value?.unassigned_devices.length ?? 0 }}</span>
            <span class="summary-badge">当前设备 {{ workspace.selectedDevice.value?.device_mac ?? "无" }}</span>
          </div>
        </div>
        ...
        <div class="member-topology-layout">
          <div class="member-topology-layout__main">
          ...
          <div class="member-topology-layout__side">
...
.member-topology-panel {
  gap: 24px;
}

.member-topology-layout {
  display: grid;
  gap: 24px;
...
.member-topology-layout__main,
.member-topology-layout__side {
  min-width: 0;
}
