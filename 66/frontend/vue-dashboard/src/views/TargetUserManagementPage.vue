...
const localPreviewError = ref("");
const localPreviewUsingBrowser = ref(false);
...
>([]);
let localBrowserStream: MediaStream | null = null;
let localAnalyzeTimer: number | null = null;
let localAnalyzeInFlight = false;
let browserSnapshotUrl = "";
...
const shouldUseTargetOnly = computed(() => users.value.length > 0);
...
const snapshotUrl = computed(() => {
  if (analyzeSource.value === "local" && browserSnapshotUrl) {
    return browserSnapshotUrl;
  }
  const localSnapshot =
...
const rawPreviewUrl = computed(() => {
  if (analyzeSource.value === "local" && localPreviewUsingBrowser.value) {
    return "";
  }
  const base =
    analyzeSource.value === "local"
...
const processedPreviewUrl = computed(() => {
  if (analyzeSource.value === "local") {
    return snapshotUrl.value;
  }
...
const annotatedPreviewUrl = computed(() => {
  const fall = analyzeResult.value?.fall_result;
  if (!fall?.annotated_image_b64) return "";
  return `data:${fall.annotated_image_mime || "image/jpeg"};base64,${fall.annotated_image_b64}`;
});
...
function retryLivePreview() {
  livePreviewMode.value = analyzeSource.value === "local" ? "stream" : "stream";
  livePreviewVersion.value += 1;
  livePreviewError.value = "";
  if (analyzeSource.value === "local") {
    void ensureLocalBrowserPreview();
  }
}

async function ensureLocalBrowserPreview() {
  if (typeof navigator === "undefined" || !navigator.mediaDevices?.getUserMedia) {
    localPreviewUsingBrowser.value = false;
    localPreviewError.value = "当前浏览器不支持直接采集设备摄像头。";
    return;
  }
  if (localBrowserStream) {
    attachLocalPreview(localBrowserStream);
    return;
  }
  try {
    ...
    localBrowserStream = stream;
    attachLocalPreview(stream);
  } catch (error) {
    localPreviewUsingBrowser.value = false;
    localPreviewReady.value = false;
    localPreviewError.value = formatError(error, "无法打开当前设备后摄像头，请检查权限。");
  }
}
...
function attachLocalPreview(stream: MediaStream) {
  ...
  video
    ...
    .then(() => {
      ...
      localPreviewUsingBrowser.value = true;
      localPreviewError.value = "";
    })
    .catch((error) => {
      localPreviewReady.value = false;
      localPreviewUsingBrowser.value = false;
      localPreviewError.value = formatError(error, "浏览器摄像头预览播放失败。");
    });
}

function stopLocalBrowserPreview() {
  if (localAnalyzeTimer != null) {
    window.clearInterval(localAnalyzeTimer);
    localAnalyzeTimer = null;
  }
  ...
  if (localBrowserStream) {
    for (const track of localBrowserStream.getTracks()) {
      track.stop();
    }
  }
  localBrowserStream = null;
  localPreviewReady.value = false;
  localPreviewUsingBrowser.value = false;
}

async function captureBrowserFrameBlob() {
  const video = localVideoRef.value;
  if (!video || video.videoWidth <= 0 || video.videoHeight <= 0) return null;
  const width = Math.min(video.videoWidth, 640);
  const height = Math.max(1, Math.round((video.videoHeight * width) / Math.max(1, video.videoWidth)));
  ...
  if (!blob) return null;
  if (browserSnapshotUrl) {
    URL.revokeObjectURL(browserSnapshotUrl);
  }
  browserSnapshotUrl = URL.createObjectURL(blob);
  return blob;
}

async function runBrowserCameraAnalysis(options?: { auto?: boolean }) {
  if (localAnalyzeInFlight || !localPreviewReady.value) return;
  localAnalyzeInFlight = true;
  if (!options?.auto) {
    analyzeBusy.value = true;
  }
  try {
    const blob = await captureBrowserFrameBlob();
    if (!blob) {
      localPreviewError.value = "浏览器摄像头画面尚未就绪，暂时无法分析。";
      return;
    }
    analyzeResult.value = await api.runBrowserFrameTargetPoseDetect(blob, {
      target_only: shouldUseTargetOnly.value,
      session_id: options?.auto ? "browser-camera-auto" : "browser-camera-manual",
      mode: "annotated",
      speed_mode: "balanced",
    });
    trendItems.value = [
      ...trendItems.value.slice(-7),
      {
        ...
        confidence: Number(
          analyzeResult.value?.posture_event?.confidence ??
            ...
            analyzeResult.value?.fall_result?.fall_score ??
            0,
        ),
    ...
    const message = formatError(error, "浏览器后摄像头画面分析失败。");
    if (options?.auto) {
      localPreviewError.value = message;
    } else {
      externalHealthError.value = message;
    }
  } finally {
    localAnalyzeInFlight = false;
    if (!options?.auto) {
...
function syncLocalAnalyzeTimer() {
  if (localAnalyzeTimer != null) {
    window.clearInterval(localAnalyzeTimer);
    localAnalyzeTimer = null;
  }
  if (analyzeSource.value !== "local" || !localPreviewUsingBrowser.value) return;
  localAnalyzeTimer = window.setInterval(() => {
    void runBrowserCameraAnalysis({ auto: true });
  }, 1500);
}
...
async function runPoseAnalysis() {
  if (analyzeSource.value === "local" && localPreviewUsingBrowser.value) {
    await runBrowserCameraAnalysis();
    return;
  }
  analyzeBusy.value = true;
  try {
    ...
    livePreviewMode.value = analyzeSource.value === "local" ? "snapshot" : "stream";
    livePreviewVersion.value += 1;
    livePreviewError.value = "";
...
onMounted(() => {
  ...
  if (analyzeSource.value === "local") {
    void ensureLocalBrowserPreview().then(() => syncLocalAnalyzeTimer());
  }
});
...
watch(
  () => localPreviewUsingBrowser.value,
  () => syncLocalAnalyzeTimer(),
);
    ...
    <section class="target-pose-grid">
      ...
      <article class="panel target-pose-preview-panel">
        ...
        <div class="target-pose-preview-panel__status">
          ...
          <span v-if="analyzeSource === 'local'" class="status-tag tone-neutral">本机摄像头优先单帧检测</span>
        ...
        <div class="target-pose-preview-grid">
          <figure class="target-pose-preview-card">
            ...
            <div class="target-pose-preview-card__frame">
              <video
                v-if="analyzeSource === 'local' && localPreviewUsingBrowser"
                ref="localVideoRef"
              ...
              />
              <img
                v-else-if="livePreviewMode === 'stream'"
                class="target-pose-preview-card__image"
                :src="rawPreviewUrl"
                alt="实时原始画面"
            ...
            <p v-if="analyzeSource === 'local' && localPreviewError" class="helper-copy">
              {{ localPreviewError }}
            </p>
          </figure>
          ...
          <figure class="target-pose-preview-card">
            ...
            <div class="target-pose-preview-card__frame">
              <img
                v-if="analyzeSource === 'local' && annotatedPreviewUrl"
                class="target-pose-preview-card__image"
                :src="annotatedPreviewUrl"
                alt="浏览器后摄像头分析标注结果"
              />
              <img
                v-else-if="livePreviewMode === 'stream' && analyzeSource !== 'local'"
                class="target-pose-preview-card__image"
      ...
      <article class="panel target-pose-visual-panel">
        ...
        <img
          v-if="analyzeSource !== 'local'"
          class="target-pose-skeleton-stream"
          :src="skeletonPreviewUrl"
          alt="姿态骨架处理后画面"
        />
        <div v-else class="target-pose-preview-card__empty">
          <strong>本机摄像头模式不常驻骨架流</strong>
          <p>当前优先保证本机实时预览和单帧跌倒检测稳定，骨架结果会在执行分析后通过下方结论区域返回。</p>
        </div>
      </article>
