...
All commands below must run in conda env `health`.
...
```powershell
conda run -n health pytest tests\test_chat_api.py
```

## Print the active interpreter explicitly

```powershell
conda run -n health python -c "import sys; print(sys.executable)"
```

Expected output:

```text
C:\Users\13010\anaconda3\envs\health\python.exe
```
