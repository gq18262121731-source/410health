...
from backend.ml.preprocess import DataValidationError, VALUE_RANGES
...
class HealthScoreService:
    ...
    def score(self, request: HealthScoreRequest) -> HealthScoreResponse:
        ...
        self._validate_realtime_vitals(request)
    ...
    def _validate_realtime_vitals(self, request: HealthScoreRequest) -> None:
        for column, (minimum, maximum) in VALUE_RANGES.items():
            raw_value = getattr(request, column, None)
            try:
                ...
                raise ServiceError(
                    ...
                    status_code=400,
                    details={"field": column, "value": raw_value},
                ) from exc
            if not minimum <= numeric_value <= maximum:
                raise ServiceError(
                    code="VALIDATION_ERROR",
                    message=f"{column} is out of range [{minimum}, {maximum}]",
                    status_code=400,
                    details={"field": column, "value": numeric_value},
                )
